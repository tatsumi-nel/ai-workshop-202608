...
class GestureArena:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key in {pygame.K_ESCAPE, pygame.K_q}:
                    ...
                    self.reset()
                elif event.key == pygame.K_SPACE and (not self.started or self.game_over):
                    if self.game_over:
                        self.reset()
                    self.started = True
    ...
    def _start_camera(self) -> None:
        ...
        if self.camera_worker is not None:
            return
        self.camera_message = f"CAMERA {self.camera_index} STARTING..."
    ...
    def _update(self, dt: float) -> None:
        ...
        if self.camera_worker is not None:
            snapshot = self.camera_worker.snapshot()
            self.reading = snapshot.reading
            self.preview = snapshot.preview
            if snapshot.status == "OPENING CAMERA...":
                self.camera_message = f"CAMERA {self.camera_index} OPENING..."
            else:
                self.camera_message = snapshot.status.replace(
                    "CAMERA",
                    f"CAMERA {self.camera_index}",
                    1,
                )
            if self.reading.gesture is not Gesture.NONE:
                self.player_x += (self.reading.x * ARENA_WIDTH - self.player_x) * min(1.0, dt * 10)
            if self.started and self.reading.gesture is Gesture.FIST:
                self._fire()
        ...
        if self.started and keys[pygame.K_SPACE]:
            self._fire()
        if keys[pygame.K_s]:
            self.shield = True
        elif self.camera_worker is None or self.reading.gesture is not Gesture.OPEN:
            self.shield = False
    ...
    def _draw(self) -> None:
        ...
        if self.game_over:
            ...
            retry = self.font.render("SPACE TO RETRY  •  R TO RESET", True, WHITE)
            ...
            shade = pygame.Surface((ARENA_WIDTH, SCREEN_HEIGHT), pygame.SRCALPHA)
            shade.fill((4, 6, 20, 150))
            self.screen.blit(shade, (0, 0))
            title = self.big.render("READY", True, CYAN)
            start = self.font.render("SPACE TO START", True, WHITE)
            self.screen.blit(title, title.get_rect(center=(ARENA_WIDTH // 2, SCREEN_HEIGHT // 2 - 20)))
            self.screen.blit(start, start.get_rect(center=(ARENA_WIDTH // 2, SCREEN_HEIGHT // 2 + 42)))
        pygame.display.flip()
    ...
    def _draw_panel(self) -> None:
        ...
        if self.preview is not None:
            ...
            if self.camera_message == "KEYBOARD MODE":
                preview_lines = ["Camera is off", "Press C to connect"]
            elif "OFFLINE" in self.camera_message:
                preview_lines = ["Check macOS camera permission", "Press C to retry"]
            else:
                preview_lines = ["Camera is starting...", "Keyboard remains available"]
            for index, line in enumerate(preview_lines):
                text = self.small.render(line, True, (120, 145, 170))
                self.screen.blit(text, text.get_rect(center=(panel_x + 150, 190 + index * 27)))
